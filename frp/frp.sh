...
detect_system(){
  if [ -f /etc/openwrt_release ] || ( [ -f /etc/os-release ] && grep -qi openwrt /etc/os-release 2>/dev/null ); then SYSTEM="openwrt";
  elif command -v systemctl >/dev/null 2>&1 && ( [ -f /etc/debian_version ] || { [ -f /etc/os-release ] && grep -qiE "debian|ubuntu" /etc/os-release 2>/dev/null || true; } ); then SYSTEM="debian";
  else
...
check_tools(){
  ...
  command -v tar >/dev/null 2>&1 || { echoerr "未找到 tar，请先安装。"; exit 1; }
}
...
get_latest_release_info(){
  ...
  tag=$(printf "%s" "$json" | grep -Eo '"tag_name":[^,]+' | sed -E 's/.*"tag_name": *"([^"]+)".*/\1/' | head -n1 || true)
  LATEST_TAG=${tag:-}
...
download_and_extract(){
  ...
  archive="${ASSET_URL##*/}"
  echoinfo "下载 $archive ..."
  if [ "$DOWNLOADER" = "curl" ]; then curl -L -o "$archive" "$ASSET_URL"; else wget -q -O "$archive" "$ASSET_URL"; fi
  mkdir -p extracted
  tar -xzf "$archive" -C extracted
  extracted_dir=$(ls -1d extracted/* 2>/dev/null | head -n1 || true)
  [ -n "$extracted_dir" ] || { echoerr "解压后未找到目录"; return 1; }
  echoinfo "解压目录: $extracted_dir"
}

ensure_usrbin_writable(){
  [ -d "$BIN_DEST" ] || mkdir -p "$BIN_DEST" 2>/dev/null || true
...
install_from_extracted(){
  ...
  [ "$sel" = "frpc" ] || [ "$sel" = "all" ] && [ -n "$frpc_src" ] && { cp -f "$frpc_src" "$BIN_DEST/frpc" 2>/dev/null && chmod 0755 "$BIN_DEST/frpc" 2>/dev/null || { echoerr "写入 $BIN_DEST/frpc 失败"; return 1; }; echoinfo "已写入 $BIN_DEST/frpc"; }
  [ "$sel" = "frps" ] || [ "$sel" = "all" ] && [ -n "$frps_src" ] && { cp -f "$frps_src" "$BIN_DEST/frps" 2>/dev/null && chmod 0755 "$BIN_DEST/frps" 2>/dev/null || { echoerr "写入 $BIN_DEST/frps 失败"; return 1; }; echoinfo "已写入 $BIN_DEST/frps"; }
  ...
  if [ "$sel" = "frpc" ] || [ "$sel" = "all" ]; then
    if [ -n "$frpc_toml_src" ]; then cp -f "$frpc_toml_src" "$ETC_DIR/frpc.toml" && chmod 0644 "$ETC_DIR/frpc.toml" || true; echoinfo "复制 frpc.toml -> $ETC_DIR/frpc.toml"; else cat >"$ETC_DIR/frpc.toml" <<'EOF'
# frpc.toml 占位
EOF
      chmod 0644 "$ETC_DIR/frpc.toml" || true; echowarn "已创建占位 $ETC_DIR/frpc.toml"; fi
  fi

  if [ "$sel" = "frps" ] || [ "$sel" = "all" ]; then
    if [ -n "$frps_toml_src" ]; then cp -f "$frps_toml_src" "$ETC_DIR/frps.toml" && chmod 0644 "$ETC_DIR/frps.toml" || true; echoinfo "复制 frps.toml -> $ETC_DIR/frps.toml"; else cat >"$ETC_DIR/frps.toml" <<'EOF'
# frps.toml 占位
EOF
      chmod 0644 "$ETC_DIR/frps.toml" || true; echowarn "已创建占位 $ETC_DIR/frps.toml"; fi
  fi

  [ "$SYSTEM" = "openwrt" ] && echoinfo "OpenWrt: 不复制二进制到 /etc/frp，init 将直接使用系统 frps/frpc 并读取 /etc/frp/*.toml"
  echoinfo "/etc/frp 已重建"
}

create_systemd_unit_files(){
  sel="$1"
  echoinfo "生成 systemd unit..."
  if [ "$sel" = "frps" ] || [ "$sel" = "all" ]; then
    [ -x "$BIN_DEST/frps" ] && cat > /etc/systemd/system/frps.service <<'EOF'
[Unit]
...
EOF
  ...
  if [ "$sel" = "frpc" ] || [ "$sel" = "all" ]; then
    [ -x "$BIN_DEST/frpc" ] && cat > /etc/systemd/system/frpc.service <<'EOF'
[Unit]
...
EOF
      chmod +x /etc/init.d/frps || true; echoinfo "/etc/init.d/frps (procd) 已生成"
    fi
    if [ "$sel" = "frpc" ] || [ "$sel" = "all" ]; then
...
EOF
      chmod +x /etc/init.d/frpc || true; echoinfo "/etc/init.d/frpc (procd) 已生成"
    fi
  else
    if [ "$sel" = "frps" ] || [ "$sel" = "all" ]; then
      cat > /etc/init.d/frps <<'EOF'
...
START=99
STOP=10
start(){ command -v frps >/dev/null 2>&1 || return 1; [ -f /etc/frp/frps.toml ] || return 1; sh -c 'frps -c /etc/frp/frps.toml 2>>/var/log/frps.log &' || true; }
stop(){ pkill -f "frps -c /etc/frp/frps.toml" 2>/dev/null || true; }
EOF
      chmod +x /etc/init.d/frps || true; echoinfo "/etc/init.d/frps (fallback) 已生成"
    fi
    if [ "$sel" = "frpc" ] || [ "$sel" = "all" ]; then
      cat > /etc/init.d/frpc <<'EOF'
#!/bin/sh /etc/rc.common
START=99
STOP=10
start(){ command -v frpc >/dev/null 2>&1 || return 1; [ -f /etc/frp/frpc.toml ] || return 1; sh -c 'frpc -c /etc/frp/frpc.toml 2>>/var/log/frpc.log &' || true; }
stop(){ pkill -f "frpc -c /etc/frp/frpc.toml" 2>/dev/null || true; }
EOF
      chmod +x /etc/init.d/frpc || true; echoinfo "/etc/init.d/frpc (fallback) 已生成"
    fi
  fi
}
...
deploy_self(){
  ...
  [ -f "$0" ] && cp -f "$0" "$target" 2>/dev/null && chmod +x "$target" 2>/dev/null && echoinfo "脚本已复制到 $target" || echowarn "复制脚本到 $target 失败（可能无权限）"
}
...
install_flow(){
  ...
  [ "$have_frps" = "yes" ] && ver_gt "$LATEST_VERSION" "$inst_frps_ver" && should_prompt_update="yes"
  [ "$have_frpc" = "yes" ] && ver_gt "$LATEST_VERSION" "$inst_frpc_ver" && should_prompt_update="yes"

  if [ "$should_prompt_update" = "yes" ]; then
    printf "\n检测到可用更新：\n"; [ "$have_frps" = "yes" ] && printf "  frps: %s -> %s\n" "$inst_frps_ver" "$LATEST_VERSION"; [ "$have_frpc" = "yes" ] && printf "  frpc: %s -> %s\n" "$inst_frpc_ver" "$LATEST_VERSION"
    printf "\n是否仅更新二进制（不会更改 /etc/frp 下 toml）？(yes/[no]) "; read ans
    if [ "$ans" = "yes" ]; then [ -n "${ASSET_URL:-}" ] || { echoerr "未获取下载地址"; return 1; }; download_and_extract || return 1
      sel="none"; [ "$have_frpc" = "yes" ] && sel="frpc"; [ "$have_frps" = "yes" ] && sel="frps"; [ "$have_frpc" = "yes" ] && [ "$have_frps" = "yes" ] && sel="all"
      install_from_extracted "$sel" || return 1; echoinfo "二进制更新完成。"; deploy_self; return 0
    fi
  fi

  if [ "$have_frps" = "no" ] || [ "$have_frpc" = "no" ]; then
    printf "\n请选择安装目标：\n 1) frps\n 2) frpc\n 3) all\n 0) 取消\n请选择: "; read choice
    case "$choice" in 1) INSTALL_TARGET="frps" ;; 2) INSTALL_TARGET="frpc" ;; 3) INSTALL_TARGET="all" ;; 0) echoinfo "已取消安装"; return 0 ;; *) echowarn "无效选择"; return 0 ;; esac
...
manage_menu(){
  while :; do
    ...
    printf "\n  %-4s %-22s\n\n" "0)" "返回主菜单"
    printf "请选择: "; read sel
    case "$sel" in 1) service_manage "frps" ;; 2) service_manage "frpc" ;; 0) break ;; *) echowarn "无效输入"; sleep 1 ;; esac
  done
}

uninstall_all(){
  clear; printf "%b警告：此操作将删除 frp 二进制、/etc/frp、服务文件与脚本！%b\n\n" "$C_WARN" "$C_RST"
  printf "确认卸载并删除所有 frp 文件？（输入 yes 确认）: "; read confirm
  [ "$confirm" = "yes" ] || { echoinfo "已取消卸载"; return 0; }
  detect_system || true
  if [ "$SYSTEM" = "debian" ]; then
    systemctl stop frps.service 2>/dev/null || true; systemctl stop frpc.service 2>/dev/null || true
    systemctl disable frps.service 2>/dev/null || true; systemctl disable frpc.service 2>/dev/null || true
    rm -f /etc/systemd/system/frps.service /etc/systemd/system/frpc.service; systemctl daemon-reload || true
  else
    /etc/init.d/frps stop 2>/dev/null || true; /etc/init.d/frpc stop 2>/dev/null || true
    /etc/init.d/frps disable 2>/dev/null || true; /etc/init.d/frpc disable 2>/dev/null || true
    rm -f /etc/init.d/frps /etc/init.d/frpc
  fi
  rm -f "$BIN_DEST/frps" "$BIN_DEST/frpc" || true; rm -rf "$ETC_DIR" || true
  rm -f /var/log/frps.log /var/log/frpc.log /var/log/frp.log || true
  if [ "$SYSTEM" = "openwrt" ]; then target="/usr/sbin/$PROG_NAME"; else target="/usr/local/bin/$PROG_NAME"; fi
  rm -f "$target" || true
  echoinfo "卸载完成。"; printf "\n按回车返回..."; read dum || true
}
